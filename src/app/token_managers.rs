...
pub fn router<S: Send + Sync + Clone + 'static>() -> Result<axum::Router<S>, ConfigurationError> {
    let router = axum::Router::new()
        ...
        .nest("/supported_assets", supported_assets::router()?);
...
async fn get_token_manager(Path(token_manager_id): Path<String>) -> impl IntoResponse {
    // ...
}

async fn update_token_manager(Path(token_manager_id): Path<String>) -> impl IntoResponse {
    // ...
}

async fn delete_token_manager(Path(token_manager_id): Path<String>) -> impl IntoResponse {
    // ...
}
